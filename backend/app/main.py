...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info("Loaded Hugging Face model: %s", hf_model.name)
        ...
        if not os.path.isdir(gen_path):
            logger.error("%s not found; run python -m app.models.download_hf_generative", gen_path)
            raise FileNotFoundError('Generative model files missing.')

        logger.info("All models and services ready")
