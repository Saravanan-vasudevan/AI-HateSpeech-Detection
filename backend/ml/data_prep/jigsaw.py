...
raw_dataset = load_dataset("jigsaw_toxicity_pred", split="train[:10000]")
df = pd.DataFrame(raw_dataset)

def map_label(row):
    toxic_score = row['toxicity']
    if toxic_score >= 0.7:
        return "hate"
...
df = df[["text", "toxicity"]]
df["label_text"] = df.apply(map_label, axis=1)
...
example = "I absolutely despise people who do that."
